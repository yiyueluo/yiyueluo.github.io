...
const OP_SPLIT_FRONT_TO_FRONT_VIA_SLIDER_L3 = {color:146};
const OP_SPLIT_FRONT_TO_FRONT_VIA_SLIDER_R3 = {color:147};
const OP_SPLIT_BACK_TO_BACK_VIA_SLIDER_L3 = {color:148};
const OP_SPLIT_BACK_TO_BACK_VIA_SLIDER_R3 = {color:149};
...
let KNITOUT_HEADERDATA = {'Position':'center', 'Width':540};
...
(function knitoutToPasses() {
	...
	(function readHeaders(){
		//TODO 
		//Currently handles position
		lines.forEach(function(line, lineIdx){
			//is header:
			let isHeader = (line[0] == ';' && line[1] == ';');
			if (!isHeader) return;
			...
			if (tokens.length == 0) return; //empty line, ignore

			let header = tokens.shift();
			let args = tokens;
			switch(header.toLowerCase()){
				...
				case 'width:':
					if(args.length && args[0].length){
						if(!isNaN(args[0])){
							KNITOUT_HEADERDATA.Width = parseInt(args[0]);
						}
						else{
							throw "ERROR: Width header must be a number, got: " + args[0];
						}
					}
					break;
				case 'position:':
					// position can be Left, Center, Right, and Keep
					KNITOUT_HEADERDATA.Position = 'center';
					if(args.length && args[0].length){
						let POSITION = args[0].toLowerCase().trim();
						if( POSITION !== 'left' &&
							POSITION !== 'right'&&
							POSITION !== 'keep' ){
							POSITION = 'center';
						}
						KNITOUT_HEADERDATA.Position = POSITION;
					}
					break;
			}
	...
	//if non-null, an object indicating that code is waiting for xfers to return some splits-to-sliders:
	//{
	//  splitToBed:"",
	//  splitToNeedles:{}, //which needles the splits are held on
	//  splitRacking:-3, //the racking the splits were placed onto the needles at
	//  xferRacking:1, //the racking the splits were to be removed from the needles at
	//}
	let sliderSplitsPending = null;
	...
	lines.forEach(function(line, lineIdx){
		...
		function throwIfSliderSplitsPending() {
			if (sliderSplitsPending) {
				throw "slider splits are pending, cannot '" + op + "' -- it is not split or xfer";
			}
		}

		//Handle operations:
		if (op === 'in' || op === 'inhook') {
			throwIfSliderSplitsPending();
			let cs = args;
			...
			cs.forEach(function(c){
				...
				if (op === 'out' && carriers[c].used_hook) throw "ERROR: Can't out carriers " + carriers[c].name + ", hook release pending " + hook + "."; 
			});

			if (op === 'outhook' && hook !== null) throw "ERROR: Can't outhook carriers " + cs + ", hook is already holding " + hook + ".";
			...
			//slider splits always look like:
			//split f0 bs1
			//split f4 bs5 ; some number of splits with same racking
			//...
			//xfer bs1 f2 ; must all be returned
			//xfer bs1 f2
			if (sliderSplitsPending) {
				if (cs.length === 0) {
					//must be a return
				} else {
					//must be another split with the same racking
...
(function passesToRaster() {
	...
	(function writePosition(){
		...
		let position = 0;
		const WIDTH = KNITOUT_HEADERDATA.Width; //Width of the machine bed
		if(KNITOUT_HEADERDATA.Position === 'center'){
			position = Math.round((WIDTH - (maxSlot - minSlot + 1)) / 2);
		}
		else if (KNITOUT_HEADERDATA.Position === 'keep'){
			if(minSlot > 0 && minSlot < (WIDTH - (maxSlot - minSlot+1))){
				position = minSlot;
			}
			else{
				console.assert("Position not in valid range [1," + WIDTH + "]. Got ["+minSlot+","+maxSlot+"]");  
			}
			
		}
		else if (KNITOUT_HEADERDATA.Position === 'right'){
		...
		else if (KNITOUT_HEADERDATA.Position === 'left'){
			position = 1;
		}
	...
	passes.forEach(function(pass) {
		...
		if (pass.type === TYPE_SPLIT_VIA_SLIDERS) {
			//nothing needed, default is sliders
		...
		if (pass.type === TYPE_XFER || pass.type === TYPE_XFER_TO_SLIDERS || pass.type === TYPE_XFER_FROM_SLIDERS) {
			...
			//options.R6 = 1; // not explicitly needed if R13 is not 100
			//options.R13 = 100; // if we want to apply stitch number to transfer pass 
		}
